myApp.controller('graphController', ['$scope', 'd3Test', 'nvTest', 'dataFetch', 'dropDownData',function($scope, d3Test, nvTest, dataFetch, dropDownData) {
    $scope.data2 =[];
    $scope.data = [];
    ...
    $scope.graphOnClick = function() {
        return function(chart) {
            chart.legend.dispatch.on('legendClick', function(event) {
                $scope.showText=true;
                event.color= event.color.replace(',60%)', ',20%)');
                d3.selectAll("#pieGraph svg").style({"display": "block"});  
                $scope.data2 = event.revenue;
                $scope.name= event.key;
            });
            chart.legend.dispatch.on('legendMouseout', function(event) {
                 event.color= event.color.replace(',20%)', ',60%)');
             });  
        } 
    }
