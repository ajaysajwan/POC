/*
A custom directive to display graph using nvd3.js.
Chart -type is two way binded using directives two-way bimding.
Based on the chart tye selected variuos types of graphs are drawn.
Isolate scope bindings two way,one way and function bindings used for customization.
*/
myApp.directive('graphNv', ['d3Test', 'nvTest', '$interval', function(d3Test, nvTest, $interval) {
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      update = function() {
        d3.select('#nvGraph svg').datum(scope.data).call(chart);
      };
      ...
      function drawChart(chartType) {
        nv.addGraph({
          generate: function() {
            chart = nv.models[chartType]().showLegend(true).showYAxis(true).showXAxis(true);
            ...
            chart.tooltip.contentGenerator(function(d) {
              if (d.data) {
                ...
                return '<div><span class="hotel-data">Hotel Name:</span>' + d.series[0].key +
                  '</div>' + '<div>' + '<span class="hotel-data">Hotel Venue:</span>' + d.series[
                    0].veneu + '</div>' + '<span class="hotel-data">Bookings:</span>' + (d.point
                    .label || d.point.x) + '<span class="hotel-data">' + d.point.y.toFixed(
                    2) + '</span>' + '<div> <span class="hotel-data">Image: </span>' +
                  '<img src=' + d.series[0].imgLocation + '>' + '</div>';
              }
            });
            ...
            chart.xAxis.axisLabel('Month').tickValues([0, 1, 2, 3, 4]).tickFormat(function(d) {
